...
func TestEncodingCompatibility(t *testing.T) {
	...
	err = enc.Encode(func(c *api.Chunk) error {
		return dec.Decode(c)
	})
...
func TestEncoderErr(t *testing.T) {
	...
	enc := newEncoder(r)
	err := enc.Encode(func(c *api.Chunk) error { return cerr })
